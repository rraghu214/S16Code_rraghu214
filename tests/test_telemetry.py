...
from s16code.telemetry import build_span_tree, export_run
from s16code.telemetry.spans import (
    COST,
...
def test_content_capture_is_off_by_default(journal, monkeypatch):
    monkeypatch.delenv("S16_OTEL_CAPTURE_CONTENT", raising=False)
...
def test_the_env_switch_also_opts_in(journal, monkeypatch):
    monkeypatch.setenv("S16_OTEL_CAPTURE_CONTENT", "1")
...
def test_no_endpoint_means_nothing_goes_over_the_wire(journal, monkeypatch):
    monkeypatch.delenv("S16_OTEL_EXPORTER_ENDPOINT", raising=False)
    export = export_run(journal)
...
def test_the_endpoint_is_read_from_config(monkeypatch):
    monkeypatch.setenv("S16_OTEL_EXPORTER_ENDPOINT", "")
...
def test_service_name_is_configurable(journal, monkeypatch):
    monkeypatch.setenv("S16_OTEL_SERVICE_NAME", "custom-service")
