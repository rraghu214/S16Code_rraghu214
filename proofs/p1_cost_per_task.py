...
**C budget-aware**    starts on the rung the ladder's ``role_tiers`` declares and
                      ...
                      admitted, metered and possibly downgraded by the S16Code
                      hard controller.

Every strategy runs through :class:`~s16code.economics.BudgetedGateway`, so all
...
:mod:`s16code.evals`. The task set is a data file; swap it for your own:
...
from s16code.economics import (  # noqa: E402
    BudgetedGateway,
    BudgetRefused,
    EconomicsConfig,
    MeteredTransport,
...
from s16code.evals import EvalsConfig, RubricJudge, Verdict, load_tasks  # noqa: E402
from s16code.evals.judge import STATUS_JUDGE_FAILED  # noqa: E402
from s16code.evals.tasks import EvalTask, by_difficulty  # noqa: E402
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--config-dir", default=os.getenv("S16_CONFIG_DIR"))
...
def run(parsed: argparse.Namespace) -> Proof:
    ...
    if parsed.judge_provider:
        from s16code.evals import JudgeModel
