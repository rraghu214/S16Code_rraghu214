...
6. **The trace really landed.** Steps 1–5 all read the spans this process built,
   ...
   The query URL is ``S16_TRACE_QUERY_URL`` if set, and otherwise derived from the
   ...
   passes, because a collector is a deployment choice and not an invariant.
    ...
    S16_TRACE_QUERY_URL=http://jaeger.internal:16686 python proofs/p4_trace_export.py --task "<anything>"
"""
...
def query_base(args: Args) -> str | None:
    ...
    explicit = (os.getenv("S16_TRACE_QUERY_URL") or "").strip()
    if explicit:
