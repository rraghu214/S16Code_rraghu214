"""Run the twenty channel prompts through live GLC v5 -> S16 services.
...
def _args() -> argparse.Namespace:
    ...
    parser.add_argument("--s16", default=os.getenv("S16_BASE_URL", "http://127.0.0.1:8123"))
...
async def _send(glc: str, token: str, scenario: ChannelScenario, *, text: str,
    ...
    envelope = {
        ...
        "thread_id": thread_id,
        # GLC overwrites this claim from its pairing store before S16 sees it.
...
async def _run_record(s16: httpx.AsyncClient, channel: str, message_id: str) -> tuple[dict, dict]:
    history = (await s16.get("/v1/agent/events")).json()["events"]
    ...
    run = (await s16.get(f"/v1/agent/runs/{decision['run_id']}")).json()
    return record, run


async def run_one(glc: str, token: str, s16: httpx.AsyncClient,
    ...
    record, run = await _run_record(s16, scenario.channel, message_id)
    ...
    if scenario.follow_up:
        ...
        follow_record, run = await _run_record(s16, scenario.channel, follow_id)
...
async def main() -> int:
    ...
    async with httpx.AsyncClient(base_url=args.glc, timeout=20) as glc, \
               httpx.AsyncClient(base_url=args.s16, timeout=20) as s16:
        ...
        for ordinal, scenario in enumerate(selected, 1):
            try:
                result = await run_one(args.glc, args.install_token, s16, scenario, ordinal)
