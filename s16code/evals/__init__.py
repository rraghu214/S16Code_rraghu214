...
* the **rubric** is fixed, generic and config-owned (:mod:`s16code.evals.config`).
...
* the **task** is data (:mod:`s16code.evals.tasks`). A task file may carry a short
...
:mod:`s16code.evals.judge` runs that rubric through the same gateway contract the
