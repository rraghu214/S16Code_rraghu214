"""The S16Code service: one FastAPI app, one importable package.
...
from s16code import a2a_routes, routes  # noqa: E402
from s16code.core.a2a.official import OfficialA2AServer  # noqa: E402
...
PORT = int(os.getenv("S16_PORT", "8113"))
...
async def lifespan(app: FastAPI):
    ...
    card = {
        "name": "S16 general live-graph agent",
        ...
        "supportedInterfaces": [
            ...
            {"url": f"dns:///127.0.0.1:{int(os.getenv('S16_A2A_GRPC_PORT', '8114'))}",
    ...
    key_file = os.getenv("S16_A2A_PRIVATE_KEY_FILE")
    self_trust = os.getenv("S16_A2A_SELF_TRUST", "0").lower() in {"1", "true", "yes"}
    ...
    if private_pem:
        kid = os.getenv("S16_A2A_SIGNING_KID", "s16-local")
        ...
        if self_trust:
            ...
            os.environ["S16_A2A_TRUSTED_KEYS_DIR"] = str(trusted_dir)

    async def handle_a2a_task(text: str) -> str:
        result = await app.state.runtime.run(
            prompt=text, scope=MemoryScope("a2a", "inbound", "remote-agent", "s16code"),
    ...
    app.state.a2a_server = A2ADemoServer(
        ...
        push_signing_secret=os.getenv("S16_A2A_PUSH_SIGNING_SECRET"), push_http=push_http,
    )
    await app.state.a2a_server.start()
    app.state.a2a_grpc_server = None
    if os.getenv("S16_A2A_GRPC_ENABLED", "1").lower() not in {"0", "false", "no"}:
        app.state.a2a_grpc_server = OfficialA2AServer(
            app.state.a2a_server, data_dir / "a2a.sqlite",
            address=f"127.0.0.1:{int(os.getenv('S16_A2A_GRPC_PORT', '8114'))}",
            bearer_tokens=bearers, api_keys=api_keys,
        )
...
app = FastAPI(title="S16Code — Live Graph, Memory, Semantic Chunking and A2A", lifespan=lifespan)
...
async def healthz(request: Request):
    return {"ok": True, "service": "s16code", "port": PORT,
